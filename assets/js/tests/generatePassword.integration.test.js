...
describe("generatePassword function", () => {
  ...
  test("should generate password of desired length", () => {
    ...
    const password = generatePassword(
      16,
      "",
      approvedCharacterSets,
      approvedCharacterSetsKeys
    );
  ...
  test("should generate password with only special characters", () => {
    ...
    for (const character of password) {
      const isSpecialCharacter =
        specialCharacters.characters.includes(character);
  ...
  test("should generate password featuring 2 character sets", () => {
    ...
    const expected = new Set([lowercaseLetters.name, uppercaseLetters.name]);
    ...
    for (const character of password) {
      ...
      if (isLowercaseLetter)
        charactersSetsInPassword.add(lowercaseLetters.name);
      else if (isUppercaseLetter)
        charactersSetsInPassword.add(uppercaseLetters.name);
      else if (isNumber) charactersSetsInPassword.add(numbers.name);
      else if (isSpecialCharacter)
        charactersSetsInPassword.add(specialCharacters.name);
